...
| 화면 | 기능 |
|------|------|
| 🏠 **운동(홈)** | 연속 인증일(streak), 오늘 인증 여부 리마인더, 참여자 오늘 상태(✅/⏳), **운동** 인증 피드(**🔥 소모 칼로리** 배지), 응원, 본인 인증 **수정** |
| 🍽️ **식단** | 날짜 이동 하루 보기, **참여자 탭 전환**(친구 식단 보기), 끼니별(아침·점심·저녁·간식) 기록, 그날 섭취 칼로리 합계, 본인 기록 **수정** |
| 📸 **인증** | 상단 **운동/식단 전환**. 카메라/사진 업로드(자동 축소). 운동: 종류·시간·**몸무게 기반 소모 칼로리 자동 계산**. 식단: 끼니 + **사진으로 AI 칼로리 추정**(선택) + **총 칼로리 직접 수정·음식 삭제** |
...
```
> `kcal` 은 식단이면 **섭취 칼로리**, 운동이면 **소모 칼로리**(MET × 몸무게 × 시간)입니다. 몸무게는 각자 기기(localStorage)에 저장되고, 저장 시점 값으로 운동의 소모 칼로리가 계산돼 기록됩니다.
